	...
	<section class="body">
		...
		<div class="inner-wrapper">
			...
			<section role="main" class="content-body kursePanel ">
				...
				<script>
					...
					//Блок групи
					$KyrsePanel1 = $connect->query("SELECT COUNT(*) as COUNT FROM `subjects` WHERE `id_ group`=(SELECT `id_ group` FROM `groups` WHERE `name` LIKE '%$kurs%') and `id_semestr`=$id_semestr[1] ORDER by `id_Subjects`					");
					while ($row = $KyrsePanel1->fetch_assoc()) {
						$num = $row["COUNT"];
					}
					$KyrsePa = $connect->query("SELECT * FROM `subjects` WHERE `id_ group`=(SELECT `id_ group` FROM `groups` WHERE `name` LIKE '%$kurs%') and `id_semestr`=$id_semestr[1] ORDER by `id_Subjects`");
		...
		<script>
			...
			function but() {
				...
				<?php
				$KyrsePanel2 = $connect->query("SELECT * FROM `subjects` WHERE `id_ group`=(SELECT `id_ group` FROM `groups` WHERE `name` LIKE '%$kurs%') and `id_semestr`=$id_semestr[1] ORDER by `id_Subjects`");
